...
const Pesticides = () => {
  ...
  const [pesticide, setPescticides] = useState([]);
  const fetchData = async () => {
    try {
      const res = await axios.get("/pesticide");
      if (res) {
        setPescticides(res.data);
  ...
  const handleDelete =async (id) => {
    const res = await axios.delete(`/pesticide/${id}/delete`)
    if(res.status===200){
      ...
      <div className=" ml-[18%] p-5">
        <div className="flex flex-col w-full">
          <div className="flex items-center justify-between w-full">
            <h2 className="text-2xl font-bold text-gray-700">Pesticides</h2>
          ...
          <div className="mt-10 text-right">
            <a href="/admin/addpesticide" className="bg-blue-600 text-white p-3 rounded-lg">
              Add Pesticides
            </a>
          </div>
                    ...
                    {pesticide.map((fert) => (
                      <tr className="text-gray-700">
                        ...
                        <td className="px-4 py-3 border">
                          <div className="flex items-center text-sm">
                            <div>
                              <p className="font-semibold">{fert.description}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3 border">
                        ...
                        <td className="px-4 py-3 border">
                          <div className="flex items-center text-sm">
                            <div className="">
                             ...
                             src={`http://100.10.10.18:8000/api/images/${fert.image}`}/>
                        ...
                        <td className="border p-3 text-center">
                          <Link
                            to={`/admin/editpesticide/${fert.id}`}
