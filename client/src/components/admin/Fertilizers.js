...
const Fertilizer = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get("/fertilizer");
      if (res) {
  ...
  const handleDelete =async (id) => {
   const res = await axios.delete(`/fertilizer/${id}/delete`)
   if(res.status===200){
    toast.success('Deleted SuccessFully !')
    navigate('/admin/fertilizers')
   }
      ...
      <div className=" ml-[18%] p-5">
        <div className="flex flex-col w-full">
          <div className="flex items-center justify-between w-full">
            <h2 className="text-2xl font-bold text-gray-700">Fertilizer</h2>
              ...
              <div className="overflow-x-auto bg-white rounded-lg shadow-xs">
                <table className="w-full whitespace-nowrap">
                  ...
                  <tbody className="bg-white">
                    {fertilizer?.map((fert) => (
                      <tr className="text-gray-700" key={fert.id}>
